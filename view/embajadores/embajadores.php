<div id="contenido" style="display: {{visible}};">
<div class="banner-embajadores">
        <!--<img  class="img-responsive" alt="1440x500" data-src="holder.js/1440x500" src="data:image/png;base64,">-->
    <div class="container padding-top-150">
    <div class="row">
        <div class="col-md-12 text-center">        
            <h2 class="text-center text-uppercase color-negro">Con Easy Dance, tú puedes ganar</h2>        
        </div>
    </div>
    </div>
    </section>
</div>
<section class=" padding-top-bottom-50">
<div class="container">
<div class="row">
    <h3 class="text-center">''HAZTE PARTE DE NUESTRO GRAN EQUIPO DE EMBAJADORES''</h3>
	<p class="padding-bottom-30 text-center">
	Nos encontramos en la búsqueda de talentos artísticos profesionales en el área del baile, 
	directores de academias, profesores o personas que hayan tenido trayectoria o sean conocedores de la materia, 
	para que logren sumarse como agente multiplicador, a través de las redes sociales para dar a demostrar la 
	nueva Marca que revolucionará el mundo del baile  en latino América. 
	</p>
	<p class="padding-bottom-30 text-center">
	 A cambio de esto el equipo de EASY DANCE, te ofrece la oportunidad de recibir descuentos, membrecías,
	 nuevas aplicaciones para tu beneficio  y cancelación económica.
	</p>
	<p class="text-center">
		"TE INVITAMOS A UNIRTE A NUESTRO EQUIPO" 
	</p>
</div>
</div>
</section>
<section class="padding-top-bottom-50 fondo-gray">
   <div class="container">
    <div class="row">
		<div class="col-md-6 text-center padding-top-10">  
			<img alt="dólares" title="dólares" src="img/iconos/dolares.png" class="img-responsive img-center img-150" width="200" >
            <h5 class="g">GANA EN DÓLARES</h5>
            <p>
			   Con sólo recomendar nuestra herramienta Easy Dance te aporta de manera mensual
			   pagos en  dólares por cada academia que subscriba nuestros servicios.
            </p>
        </div>
        <div class="col-md-6 text-center padding-top-10">  
			<img alt="dólares" title="dólares" src="img/iconos/porce.png" class="img-responsive img-center img-150" width="200" >
            <h5 class="g text-uppercase">Lo más importante para tu academia</h5>
            <p>
				Sólo recomendando la aplicación y logrando que una academia amiga se sume a nuestros planes;
				En  Easy Dance te  obsequiamos un mes gratis  por cada academia que descargue e inicie la aplicación.
				Para recomendarnos es muy sencillo.
            </p>
        </div>
		<div class="col-md-12 text-center padding-top-bottom-30">			
			<p>
				<b>Usa el siguiente y link y compártelo con tus amigos dentro o  fuera de Venezuela.</b>
			</p>
			<div class="col-md-offset-2 col-md-6">
				<a href="#/postularse"><input class="form-control color-negro text-center" type="text" name="link" id="link" value="http://easydancelatino/#/postularse" /></a>
			</div>
			<div class="col-md-2">
				 <!--<a class="btn btn-violeta btn-sm text-uppercase">Cópialo</a>-->
			</div>
		</div>
		
               
    </div>       
   </div>
</section>


</div>
<script type="text/javascript-lazy">
   loadjscssfile("js/function/cargar.js", "js");
</script>
